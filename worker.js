...
// Track last pot watering time untuk backup anti-spam
const lastPotTime = {};
...
async function checkSensorThresholds() {
  ...
  try {
    ...
    for (const thresholdKey of allThresholds) {
      ...
      // Check MEMORY THRESHOLD cooldown FIRST (fastest anti-spam)
      const lastThresholdTriggerTime = lastThresholdTime[thresholdKey];
      if (lastThresholdTriggerTime && Date.now() - lastThresholdTriggerTime < config.worker.sensorDebounce) {
        const remainingSeconds = Math.ceil((config.worker.sensorDebounce - (Date.now() - lastThresholdTriggerTime)) / 1000);
      ...
      for (const potNumber of potAktif) {
        ...
        // ============ ANTI-SPAM CHECK 3: Firebase & Memory Per-Pot Cooldown ============
        // Check memory-based cooldown FIRST (instant, most reliable anti-spam)
        const lastPotTriggerTime = lastPotTime[potNumber];
        if (lastPotTriggerTime && Date.now() - lastPotTriggerTime < config.worker.sensorDebounce) {
          const remainingSeconds = Math.ceil((config.worker.sensorDebounce - (Date.now() - lastPotTriggerTime)) / 1000);
          console.log(`      ≡ƒÜ¿ POT ${potNumber}: Memory COOLDOWN active (${remainingSeconds}s remaining) - SKIP`);
          continue; // Skip this pot
        }

        // Then check Firebase cooldown as backup
      ...
      if (potsNeedWatering.length > 0) {
        ...
        // ✅ SET MEMORY COOLDOWNS IMMEDIATELY (before Firebase async write)
        // This is the KEY FIX to prevent continuous triggering!
        lastThresholdTime[thresholdKey] = Date.now();
        for (const pot of potsNeedWatering) {
          lastPotTime[pot] = Date.now();
        }
        console.log(`   ≡ƒöÆ Memory cooldowns SET IMMEDIATELY for threshold + pots (2 min debounce)`);

        // Then set Firebase cooldowns BEFORE creating job
        try {
          ...
          console.warn(`      ΓÜá∩╕Å Cooldown write failed (will rely on memory cooldown): ${cooldownError.message}`);
          // Continue anyway - memory cooldown is already set
        }
